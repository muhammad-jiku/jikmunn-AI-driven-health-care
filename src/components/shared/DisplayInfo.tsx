...
interface Metrics {
  totalFolders: number;
  aiPersonalizedTreatment: number;
...
const DisplayInfo: React.FC = () => {
  ...
  useEffect(() => {
    if (user) {
      fetchUserByEmail(user?.email?.address as string)
        .then(() => {
            ...
            if (record.kanbanRecords) {
              try {
                ...
                aiPersonalizedTreatment += kanban.columns.some(
                  (column: { title: string }) =>
                    column.title === 'AI Personalized Treatment'
                )
                  ? 1
                  : 0;
                totalScreenings += kanban.tasks.length;
                completedScreenings += kanban.tasks.filter(
                  (task: { columnId: string }) => task.columnId === 'done'
                ).length;
                pendingScreenings += kanban.tasks.filter(
                  (task: { columnId: string }) => task.columnId === 'doing'
                ).length;
                overdueScreenings += kanban.tasks.filter(
                  (task: { columnId: string }) => task.columnId === 'overdue'
                ).length;
              } catch (error) {
  ...
  }, [fetchUserByEmail, user, fetchUserRecords, records]);
